...
UOWSPlayerControllerComponent::UOWSPlayerControllerComponent()
{
    ...
    // GameInstance will be null on Editor startup, but will have a valid reference when playing the game
    if (GameInstance)
    {
...
void UOWSPlayerControllerComponent::OnTravelToLastZoneServerResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
    ...
    if (!GetWorld())
    {
        UE_LOG(OWS, Error, TEXT("No World available!"));
        return;
    }
    ...
    if (!GameInstance)
    {
        UE_LOG(OWS, Error, TEXT("GameInstance cast failed!"));
        return;
    }

    UE_LOG(OWS, Log, TEXT("Raw response: %s"), *Response->GetContentAsString());
    UE_LOG(OWS, Log, TEXT("Response code: %d"), Response->GetResponseCode());
    UE_LOG(OWS, Log, TEXT("Success: %d"), bWasSuccessful);
    ...
    if (bWasSuccessful)
    {
        ...
        if (FJsonSerializer::Deserialize(Reader, JsonObject))
        {
            ...
            if (!bServerSuccess)
            {
                UE_LOG(OWS, Error, TEXT("Server returned success: false. Cannot travel to map."));
                return;
            }
            ...
            {
                UE_LOG(OWS, Error, TEXT("ServerIP or Port is empty!"));
                return;
            }

            int32 MapInstanceStatus = JsonObject->GetIntegerField(TEXT("mapInstanceStatus"));
            if (MapInstanceStatus != 1)
            {
                UE_LOG(OWS, Error, TEXT("Map instance status is %d, not ready for connection."), MapInstanceStatus);
                return;
            }
            ...
            FString URL = ServerAndPort + FString(TEXT("?ID=")) + EncryptedIDData;
            TravelToMap(URL, false);
        }
        else
        {
            UE_LOG(OWS, Error, TEXT("Server returned no data or invalid JSON!"));
        }
    }
    else
    {
        UE_LOG(OWS, Error, TEXT("Request failed!"));
    }
}
...
void UOWSPlayerControllerComponent::OnLaunchZoneInstanceResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
    ...
    if (bWasSuccessful)
    {
        ...
        if (FJsonSerializer::Deserialize(Reader, JsonObject))
        {
            ...
            if (ServerIP.IsEmpty() || Port.IsEmpty())
            {
                UE_LOG(OWS, Error, TEXT("OnLaunchZoneInstanceResponseReceived: ServerIP or Port is empty!"));
                OnErrorLaunchZoneInstanceDelegate.ExecuteIfBound(TEXT("Cannot connect to server!"));
                return;
            }
